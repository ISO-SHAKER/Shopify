let products = [];
let filteredProducts = [];
let cart = [];

/* ===== Selectors ===== */
let selectors = {
  productTitle: document.querySelector("#title"),
  minPrice: document.querySelector("#min"),
  maxPrice: document.querySelector("#max"),
  searchForm: document.querySelector("#search-form"),
  searchBtn: document.querySelector("#search-btn"),
  categoryBtn: document.querySelector("#category"),
  sortBtn: document.querySelector("#sort"),
  products: document.querySelector(".products"),
  cartBtn: document.querySelector(".cart-btn"),
  cartQty: document.querySelector(".cart-qty"),
  cartClose: document.querySelector(".cart-close"),
  cart: document.querySelector(".cart"),
  cartOverlay: document.querySelector(".cart-overlay"),
  cartClear: document.querySelector(".cart-clear"),
  cartBody: document.querySelector(".cart-body"),
  cartTotal: document.querySelector(".cart-total"),
};

/* ===== Event Listeners ===== */

const setupListeners = () => {
  document.addEventListener("DOMContentLoaded", initStore);

  // search event
  selectors.searchBtn.addEventListener("click", filterProducts);
  selectors.searchForm.addEventListener("click", stopRefresh);
  selectors.sortBtn.addEventListener("change", sortProducts);
  selectors.categoryBtn.addEventListener("change", filterCategories);

  // product event
  selectors.products.addEventListener("click", addToCart);

  // cart events
  selectors.cartBtn.addEventListener("click", showCart);
  selectors.cartOverlay.addEventListener("click", hideCart);
  selectors.cartClose.addEventListener("click", hideCart);
  selectors.cartBody.addEventListener("click", updateCart);
  selectors.cartClear.addEventListener("click", clearCart);
};

/* ===== Event Handlers ===== */
const initStore = () => {
  loadCart();
  loadProducts("https://fakestoreapi.com/products")
    .then(renderProducts)
    .then(renderCategories)
    .finally(renderCart);
};

const stopRefresh = (e) => {
  e.preventDefault();
};

/* ===== Filter All Products ===== */
const filterProducts = () => {
  const { productTitle, minPrice, maxPrice, categoryBtn } = selectors;
  const title = productTitle.value.toLowerCase();
  const min = parseFloat(minPrice.value) || 0;
  const max = parseFloat(maxPrice.value) || 1000000;

  console.log(title, min, max);

  filteredProducts = products.filter(
    ({ title: productTitle, price, category: productCategory }) =>
      productTitle.toLowerCase().includes(title) && price >= min && price <= max
  );
  renderProducts(filteredProducts);
};

/* ===== Sort All Products ===== */
const sortProducts = () => {
  const { sortBtn } = selectors;
  const sort = sortBtn.value;
  console.log(sort);
  if (sort === "price") {
    filteredProducts.sort((a, b) => a.price - b.price);
  } else if (sort === "title") {
    filteredProducts.sort((a, b) => a.title.localeCompare(b.title));
  } else if (sort === "category") {
    filteredProducts.sort((a, b) => a.category.localeCompare(b.category));
  }

  renderProducts(filteredProducts);
};

/* ===== Filter All Products By Categories ===== */
const filterCategories = () => {
  const { categoryBtn } = selectors;
  const category = categoryBtn.value;
  console.log(category);
  if (category === "category") {
    renderProducts(products);
    return;
  }
  filteredProducts = products.filter(
    (product) => product.category === category
  );
  renderProducts(filteredProducts);
};

const showCart = () => {
  selectors.cart.classList.add("show");
  selectors.cartOverlay.classList.add("show");
};

const hideCart = () => {
  selectors.cart.classList.remove("show");
  selectors.cartOverlay.classList.remove("show");
};

/* ===== Remove All Products From Cart ===== */
const clearCart = () => {
  cart = [];
  saveCart();
  renderCart();
  renderProducts();
};

/* ===== Add Product To Cart ===== */
const addToCart = (e) => {
  if (e.target.hasAttribute("data-id")) {
    const id = parseInt(e.target.dataset.id);
    cart.push({ id, qty: 1 });
    saveCart();
    renderProducts();
    renderCart();
    showCart();
  }
};

/* ===== Remove Product From Cart ===== */
const removeFromCart = (id) => {
  cart = cart.filter((item) => item.id !== id);
  renderProducts();
};

/* ===== Increase Quantity Of Product In Cart ===== */
const increaseQty = (id) => {
  const item = cart.find((item) => item.id === id);
  if (!item) return;

  item.qty++;
};

/* ===== Decrease Quantity Of Product In Cart ===== */
const decreaseQty = (id) => {
  const item = cart.find((item) => item.id === id);
  if (!item) return;

  item.qty--;

  if (item.qty === 0) removeFromCart(id);
};

/* ===== Update Cart-Data ===== */
const updateCart = (e) => {
  if (e.target.hasAttribute("data-btn")) {
    const cartItem = e.target.closest(".cart-item");
    const id = parseInt(cartItem.dataset.id);
    const btn = e.target.dataset.btn;

    btn === "incr" && increaseQty(id);
    btn === "decr" && decreaseQty(id);

    saveCart();
    renderCart();
  }
};

/* ===== Save Cart-Data In Local-Storage ===== */
const saveCart = () => {
  localStorage.setItem("myCart", JSON.stringify(cart));
};

/* ===== Load Cart-Data From Local-Storage ===== */
const loadCart = () => {
  cart = JSON.parse(localStorage.getItem("myCart")) || [];
};

/* ===== Display Cart-Data In Document ===== */
const renderCart = () => {
  // show cart qty in navbar
  const cartQty = cart.reduce((sum, item) => {
    return sum + item.qty;
  }, 0);

  selectors.cartQty.textContent = cartQty;
  selectors.cartQty.classList.add("visible");

  // show cart total
  selectors.cartTotal.textContent = calculateTotal().format();

  // show empty cart
  if (cart.length === 0) {
    selectors.cartBody.innerHTML =
      '<div class="cart-empty">Your cart is empty.</div>';
    return;
  }

  // show cart items
  selectors.cartBody.innerHTML = cart
    .map(({ id, qty }) => {
      // get product info of each cart item
      const product = products.find((x) => x.id === id);

      const { title, image, price } = product;

      const amount = price * qty;

      return `
        <div class="cart-item" data-id="${id}">
          <img src="${image}" alt="${title}" />
          <div class="cart-item-detail">
            <h3>${title}</h3>
            <h5>${price.format()}</h5>
            <div class="cart-item-amount">
              <i class="bi bi-dash-lg" data-btn="decr"></i>
              <span class="qty">${qty}</span>
              <i class="bi bi-plus-lg" data-btn="incr"></i>

              <span class="cart-item-price">
                ${amount.format()}
              </span>
            </div>
          </div>
        </div>`;
    })
    .join("");
};

/* ===== Display Products In Document ===== */
const renderProducts = (list = products) => {
  selectors.products.innerHTML = list
    .map((product) => {
      const { id, title, image, price } = product;

      // check if product is already in cart
      const inCart = cart.find((item) => item.id === id);

      // make the add to cart button disabled if already in cart
      const disabled = inCart ? "disabled" : "";

      // change the text if already in cart
      const text = inCart ? "Added in Cart" : "Add to Cart";

      return `
    <div class="product">
      <img src="${image}" alt="${title}" />
      <h3>${title}</h3>
      <h5>${price.format()}</h5>
      <button ${disabled} data-id=${id}>${text}</button>
    </div>
    `;
    })
    .join("");
};

/* ===== Display All Categories In Document ===== */
const renderCategories = () => {
  const categories = [...new Set(products.map((product) => product.category))];

  selectors.categoryBtn.innerHTML += categories
    .map((category) => `<option value="${category}">${category}</option>`)
    .join("");
};

/* ===== Fetch Products From Api ===== */
const loadProducts = async (url) => {
  try {
    const response = await fetch(url);
    if (!response.ok) {
      throw new Error(`http error! status=${response.status}`);
    }
    products = await response.json();
    console.log(products);
  } catch (error) {
    console.error("fetch error:", error);
  }
};

/* ===== Helper Functions ===== */
const calculateTotal = () => {
  return cart
    .map(({ id, qty }) => {
      const { price } = products.find((item) => item.id === id);

      return qty * price;
    })
    .reduce((sum, number) => {
      return sum + number;
    }, 0);
};

Number.prototype.format = function () {
  return this.toLocaleString("en-US", {
    style: "currency",
    currency: "USD",
  });
};

/* ===== Initialize Document ===== */
setupListeners();
